...
if IsDuplicityVersion() then
    function ForceOpenInventory(source, name, label, maxSlots, maxWeight, coords)
        local others = {
            maxweight = maxWeight,
            slots = maxSlots
        }
        TriggerEvent("inventory:server:OpenInventory", source, "stash", "Housing_"..name, others)
        TriggerClientEvent("inventory:client:SetCurrentStash", source, "Housing_"..name)
    end
