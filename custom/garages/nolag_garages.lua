...
if Config.Garage ~= "nolag_garages" then
    return
end
...
Config.Functions["OpenGarageMenu"] = {
    ...
    onSelect = function(property, data)
        ...
        if cache.vehicle then
            exports.nolag_garages:ParkVehicle("property_" .. property.id .. "_" .. data.options.id, "property")
        else
            local garage = {
                name = "property_" .. property.id .. "_" .. data.options.id,
                label = "House " .. property.id,
                type = "property",
                spawn = vector4(data.coords.x, data.coords.y, data.coords.z, data.coords.w),
            }
            exports.nolag_garages:OpenGarageMenu(garage)
        end
    end,
}
